...
- [ ] 🔴 **แก้ `s_epi_complete`** (วัคซีน fully immunized) — ระบบ 41.60% แต่ HDC 87.10% · `target` รายปีเป็นผลรวมรายเดือนซ้ำ ไม่ใช่เด็ก unique → ตัวหารพองผิด · KPI เกือบผ่านแต่โชว์ตก · ต้อง probe field ดิบหา cohort เด็ก unique / ถาม owner (ห้ามเดา)
