...
## 4b. ปีฐานของตัวชี้วัดแบบ "เพิ่มขึ้นเทียบปีก่อนหน้า" (#46 — บังคับ ห้ามข้าม)

ตัวชี้วัดที่ `calcMode='percentIncrease'` ต้องมี **ยอดทั้งปีของปีงบก่อนหน้า** ถึงจะคำนวณได้ (ตาราง `kpi_baseline_year`)
ระบบหาให้เองตามลำดับ: ตารางปีฐาน → snapshot เดือนสุดท้ายของปีก่อน → ค่าคงที่ใน `moph_config` (เฉพาะที่ `baseFiscalYear` ตรงปี) → **ไม่เจอ = ไม่คำนวณ + ขึ้นคำเตือน** (ไม่เดาให้)

1. **ปีฐาน 2568 (คีย์มือ ครั้งเดียวตลอดกาล)** — MOPH หยุด sync ปีนั้นไปแล้ว ดึงสดไม่ได้
   ```
   mysql ... < _resync_backup/item46-baseline-2026-08-18/02_commit.sql
   ```
   gate ต้องได้ 7 แถว · 11,069 / 1,491 / 5,315 / 950 (ตรงตาราง HDC จริงที่ owner ยืนยัน)
2. **ปีฐาน 2569 (ระบบเก็บเอง)** — ให้ batch/cron บน production รัน **อย่างน้อย 1 ครั้งก่อน 30 ก.ย. 69** ระบบจะเก็บยอดปีงบ 2569 ให้อัตโนมัติ
   · ถ้าเลย 1 ต.ค. ไปแล้วยังไม่มี ให้สั่งดึงย้อนปีแทน: `POST /api/moph/batch body:{ year:'2569', ... }` (ปีฐานจะลงช่อง 2569 ตาม `year` ที่สั่ง ไม่ใช่ตามเดือนที่กด)
3. **ล็อกยอดสิ้นปี (แนะนำ พ.ย.–ธ.ค. 69)** — สั่งดึงย้อนปี 2569 อีกครั้งเพื่อทับปีฐานด้วยยอดที่ครบกว่า (MOPH มักตามมาเติมยอดหลังปิดปี) แล้วเทียบยอดรวมกับ HDC หนึ่งครั้ง
4. **ตรวจว่าเป้าปีงบใหม่ตั้งไว้แล้ว** — #46 เกณฑ์ ปี68=10% · ปี69=15% · **ปี70=20%** (ตั้งใน `kpi_targets` แล้วทั้ง 2569/2570) · ถ้าไม่มีแถวของปีนั้น ระบบจะถอยไปใช้เป้ากลางใน `kpi_reports.target` เงียบๆ
