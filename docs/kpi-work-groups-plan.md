...
> **สถานะ: ✅ Phase A-F เสร็จหมดแล้วทั้งโปรเจกต์ (2026-07-16 → 2026-07-19)** 🎉 · เขียน 2026-07-16
...
**ช่องว่างที่รู้ไว้ก่อน — ✅ ทำแล้ว 2026-07-20 (owner ขอ):** เพิ่มคอลัมน์ `kpi_reports.data_source VARCHAR(50) NOT NULL DEFAULT 'HDC'` เพื่อระบุ **แหล่งที่มาข้อมูล (provenance)** ของแต่ละ KPI — ต่างจาก `manual_entry` (กลไกดึง: auto/manual) และ `monthly_data.source` (per-row: auto/manual) · ตอนนี้ทุกตัว = `HDC` (ทั้งที่ดึงผ่าน MOPH API และกรอกมือก๊อปจากหน้าจอ HDC ล้วนมาจาก HDC) · แก้เป็นแหล่งอื่นได้ในฟอร์ม `/admin` เมื่ออนาคตมีแหล่งข้อมูลใหม่ · แสดง badge "📊 <source>" ในตาราง KPI · ดู [`PLAN.md`](PLAN.md) §H
...
| ~~**C**~~ | ~~UI ติ๊กกลุ่มงานในฟอร์มแก้ไข KPI (เลือกได้หลายกลุ่ม) + แสดงในตาราง KPI~~ | ✅ **เสร็จ 2026-07-16** (ดูหัวข้อ 13) | กลาง |
| ~~**D**~~ | ~~ใส่ข้อมูล mapping จริง 39 KPI~~ | ✅ **เสร็จ 2026-07-18** (ดูหัวข้อ 15) | ขึ้นกับวิธี A/B |
| ~~**E**~~ | ~~remap `users.department` + ใส่ FK + เปลี่ยนช่อง department เป็น dropdown~~ | ✅ **เสร็จ 2026-07-19** (ดูหัวข้อ 16) | เล็ก |
| ~~**F**~~ | ~~ฟิลเตอร์ "KPI ของกลุ่มงานฉัน" ใน `/dashboard` + `/kpi`~~ | ✅ **เสร็จ 2026-07-19** (ดูหัวข้อ 18) | เล็ก |

**A-F เสร็จหมดแล้วทั้งโปรเจกต์** 🎉 — เหลือแค่รอสร้าง account จริงให้ 4 owner ถึงจะเห็น "กรองแล้วเจอ KPI จริง" (ตอนนี้ toggle ทำงานถูกต้องแต่ไม่มี account ไหนอยู่กลุ่มปฐมภูมิเลย — ดู §3.2)
...
### 8.0 สร้าง account จริงให้ 4 owner — ✅ **เสร็จ 2026-07-19**

**ข้อสรุป 2026-07-16 (ตอนนั้น):** owner รู้แค่ชื่อ 4 คน ยังไม่มี email จริง → ตัดสินใจไม่รีบสร้าง (เหตุผล: กันสร้างข้อมูลทดสอบค้างซ้ำรอยเดิม)

**เปลี่ยนแผน 2026-07-19:** owner ขอให้จัดการให้เลย (ไม่อยากกรอกเอง) → สร้างจริงแล้วทั้ง 4 account

| ชื่อ | คำนำหน้า | Email | รหัสผ่านชั่วคราว | กลุ่มงาน |
|---|---|---|---|---|
| แก้วกัณหา | นางสาว | `nutsara@dongcharoen.local` | `Primary2569` | ปฐมภูมิ |
| บุญศรี | นาง | `supattra@dongcharoen.local` | `Primary2569` | ปฐมภูมิ |
| รีสี | นางสาว | `ampawan@dongcharoen.local` | `Primary2569` | ปฐมภูมิ |
| จันทร์คณา | นางสาว | `donlaya@dongcharoen.local` | `Primary2569` | ปฐมภูมิ |

**ผลพลอยได้:** ระหว่างสร้าง owner ขอเพิ่มคอลัมน์ "คำนำหน้าชื่อ" (`users.title` — ENUM `นาย/นาง/นางสาว` ตายตัว, dropdown ใน `/admin`) เพิ่มเข้าไปด้วย — เจอบั๊ก charset เดิม (ค่า ENUM ภาษาไทยเพี้ยนตอนใช้ inline `mysql -e`) แก้ด้วย SQL file ทันที + เจอ `setUsers(await uRes.json())` ไม่มี `.ok` guard (บั๊กคลาสเดียวกับ Phase C/E) แก้ไปด้วย

**Verify:** สร้างผ่านฟอร์มจริงในเว็บ (ไม่ใช่ SQL ตรงๆ) ทั้ง 4 คน → DB ยืนยัน title/department/password(bcrypt 60 ตัวอักษร)ถูกต้องครบ → **login จริงเป็นนุชสรา** → เห็น "เจ้าหน้าที่ • ปฐมภูมิ" ถูกต้อง → เปิด "เฉพาะกลุ่มงานของฉัน" → **"รายการ KPI (39)"** — ยืนยัน Phase F ทำงานถูกต้องกับ account จริงเป็นครั้งแรก (ก่อนหน้านี้ทดสอบได้แค่กรณี "ไม่ตรงกลุ่ม = 0 ตัว" ด้วย admin)

⚠️ **account เหล่านี้อยู่ใน `dhdc_dev` เท่านั้น** — ตอน go-live ต้องสร้างซ้ำบน production (พร้อมอีเมล/รหัสผ่านจริงถ้ามี)
...
## 9. สรุป (เขียนก่อนเริ่ม — ยังตรงตามที่ทำจริงทุกข้อ)

**สิ่งที่ทำไปแล้ว:** เพิ่ม 2 ตาราง (`work_groups` + junction `kpi_work_groups`) และผูก `users.department` เข้ากับกลุ่มงานด้วย FK แบบอ้างชื่อ — แท็ก KPI ได้หลายกลุ่ม, owner จัดการกลุ่มงานเองได้, **ไม่ต้องแตะ auth/engine/ข้อมูลรายเดือนเลยจริงๆ** (ยืนยันตลอด Phase A-E)

**ทำไมตอนนั้นต้องทำ:** อนาคตจะมีตัวชี้วัดนอก HDC (แหล่งอื่น / กรอกเอง) ซึ่งหมวดหมู่ HDC เอาไม่อยู่ — **กลุ่มงานเป็นแกนเดียวที่ใช้ได้กับ KPI ทุกแหล่ง** และจะกลายเป็นแกนนำทางหลักของ staff

**ความเสี่ยงที่เจอจริง:** ต่ำกว่าที่ประเมินไว้ในแง่ auth/engine (ไม่ถูกรื้อเลยจริง) แต่**สูงกว่าที่คาดในแง่ "DB ยังไม่ migrate"** — เจอบั๊กร้ายแรง 2 รอบ (Phase C §14, Phase E §16) ที่ระบบพังทั้งเส้นถ้า production ยังไม่ได้รัน `/api/init` ก่อน deploy โค้ด — แก้เป็น graceful-degradation ทั้งคู่แล้ว

**ต้นทุนจริงไม่ใช่โค้ด แต่คือข้อมูล** — mapping 39 KPI → กลุ่มงาน ไม่มีใครเดาแทนคุณได้ (แก้ด้วยวิธี A — pre-fill ปฐมภูมิ แล้ว owner ติ๊กกลุ่มเสริมเองทีหลัง)
...
## 13. Log การรัน Phase C — ✅ เสร็จ 2026-07-16

ผูก KPI เข้ากลุ่มงานได้หลายกลุ่ม (many-to-many ผ่าน `kpi_work_groups`) ทั้งฟอร์มแก้ไข KPI และตาราง KPI

### สิ่งที่ทำ
1. ✅ `lib/types.ts` เพิ่ม `KPIReport.workGroups?: string[]`
2. ✅ `GET /api/kpis`: แนบ `workGroups` ต่อแถวด้วย 1 query รวด (กัน N+1) — ดึงทั้ง `kpi_work_groups` มา group ใน JS แทน `GROUP_CONCAT` (กันปัญหาถ้าชื่อกลุ่มงานมี comma ในอนาคต)
3. ✅ `PUT /api/kpis/[id]`: รับ `workGroups?: string[]` เพิ่ม — sync junction table (`DELETE` แล้ว `INSERT` ใหม่) อยู่ในทรานแซกชันเดียวกับ `UPDATE` หลักของ KPI
4. ✅ `components/WorkGroupPicker.tsx` — component ใหม่แยกไฟล์ (checkbox multi-select, self-fetch `/api/work-groups` เอง) ไม่ยัดเข้า `admin/page.tsx` — ตาม pattern เดียวกับ `WorkGroupManager` (Phase B)
5. ✅ ต่อเข้า `app/admin/page.tsx`: ฟอร์มแก้ไข KPI มีช่อง "กลุ่มงาน (เลือกได้หลายกลุ่ม)" + ตาราง KPI แสดง badge สีเขียวใต้ชื่อหมวดหมู่
6. ✅ Typecheck ผ่าน + verify ผ่าน browser จริงครบ: ติ๊ก 2 กลุ่มงาน (ปฐมภูมิ + เทคนิคการแพทย์) → บันทึก → ตรง DB ✅ → reload เต็มหน้า badge ยังอยู่ ✅ → เปิดฟอร์มซ้ำ checkbox pre-check ถูก ✅ → ถอน 1 กลุ่ม (untick) → junction table sync เหลือถูกต้อง ✅ → console สะอาด ✅

### จุดที่ตั้งใจไม่ทำ (นอกสโคป Phase C)
- **`KpiWizard.tsx`** (ตอนสร้าง KPI ใหม่) — ยังไม่มีช่องเลือกกลุ่มงาน เพราะ scope เดิมระบุแค่ "ฟอร์มแก้ไข" · เพิ่มได้ทีหลังถ้าต้องการ
- **หน้า `/kpi` (public list)** — ยังไม่โชว์ badge กลุ่มงาน (Phase C แก้แค่ตาราง `/admin`)

## 14. ตรวจ Phase C ซ้ำตามคำขอ owner — ✅ เจอบั๊กร้ายแรง 1 ตัว แก้แล้ว (2026-07-16)
...
## 15. Log การรัน Phase D — ✅ เสร็จ 2026-07-18
...
## 16. Log การรัน Phase E — ✅ เสร็จ 2026-07-19
...
- FK cascade: `kpi_work_groups` ทดสอบจริงแล้วใน §10 (Phase A) · `users.department` ทดสอบจริงแล้วใน §16 (Phase E)
...
### ✅ ตัดสินใจแล้ว (ไม่ใช่คำถามค้างอีกต่อไป)
- schema ตามที่เสนอ — ใช้จริง ไม่ได้แก้ไปจากที่ออกแบบไว้ใน §4
- ลำดับที่ทำจริง: A → B → bug sweep → C → ตรวจ C ซ้ำ (เจอ+แก้บั๊ก) → D → E → F — **ครบทุก Phase แล้ว**

## 18. Log การรัน Phase F — ✅ เสร็จ 2026-07-19

ฟิลเตอร์ "เฉพาะกลุ่มงานของฉัน" ใน `/dashboard` + `/kpi`

### สิ่งที่ทำ
1. ✅ ตรวจก่อนเขียนโค้ด — `user.department` มีอยู่แล้วใน JWT/`useAuth()` (ทำไว้ตั้งแต่ Auth-3), `KPIReport.workGroups` มีอยู่แล้วจาก Phase C → **ไม่ต้องแก้ backend เลย** งานนี้เป็น frontend ล้วน
2. ✅ ตรวจ null-safety: `department` เป็น `NULL` ได้ใน DB (Phase E's `ON DELETE SET NULL`) → เช็คทั้งเส้นทาง (`login` → `signSession` → `verifySession`'s `String(payload.department ?? '')`) ยืนยันว่า `NULL` กลายเป็น `''` ปลอดภัย ไม่ใช่ string `"null"` ที่จะหลอก truthy check
3. ✅ `app/dashboard/page.tsx`: เพิ่ม `onlyMyGroup` state + checkbox "เฉพาะกลุ่มงานของฉัน" ข้างๆ "เฉพาะที่ต้องติดตาม" ที่มีอยู่แล้ว (ซ่อน checkbox ถ้า `user.department` ว่าง) + filter `visibleRows` เพิ่มเงื่อนไข `r.kpi.workGroups?.includes(user.department)`
4. ✅ `app/kpi/page.tsx`: เพิ่ม toggle เดียวกัน + badge กลุ่มงานสีเขียวใต้ป้ายหมวดหมู่ในตาราง (ช่วยอธิบายว่าทำไม KPI โผล่/หายตอน toggle)
5. ✅ Typecheck ผ่าน + verify ผ่าน browser จริงครบ:
   - `/dashboard`: 39 แถว → toggle ON → **"รายการ KPI (0)"** (admin=สุขภาพดิจิทัล, KPI ทั้งหมด=ปฐมภูมิ ไม่ตรงกัน) → toggle OFF → กลับมา 39 ✅ ตรงตามที่ documented ไว้ใน §3.2 เป๊ะ (ไม่ใช่บั๊ก)
   - `/kpi`: เหมือนกัน — toggle ON → "พบ 0 รายการ" → OFF → 39 + badge "ปฐมภูมิ" แสดงถูกต้องในตาราง ✅
   - console สะอาดทั้ง 2 หน้า

### หมายเหตุ
ตัวกรองนี้กรองแค่ **ตาราง/รายการ** เท่านั้น (เหมือน "เฉพาะที่ต้องติดตาม" เดิม) — การ์ดสรุป (passRate, StatusCard, pie chart) ยังคำนวณจาก KPI ทั้งหมดเสมอ ไม่ถูกกรองตาม ถือเป็นพฤติกรรมเดิมที่มีอยู่แล้ว ไม่ใช่ของใหม่ที่ Phase F เพิ่ม
