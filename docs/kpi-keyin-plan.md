...
> **สถานะ: ✅ เสร็จหมดทุก Phase (H1-J + เดือนไทย + data_source + UI โหมดค่าเดียว) — commit ถึง `b79049e` push แล้ว** 🎉
> อัปเดตล่าสุด 2026-07-20 · อ่านคู่กับ [`kpi-work-groups-plan.md`](kpi-work-groups-plan.md) (ฟีเจอร์กลุ่มงานที่เป็นฐาน เสร็จหมดแล้ว)
>
> **ฟีเจอร์นี้ปิดจบแล้ว** — staff เจ้าของกลุ่มงานกรอกผลงาน KPI ตัวเองได้ครบวงจร (2 โหมด: ราย รพ.สต. 7 หน่วย / ค่าเดียว) + ล็อกเดือนปัจจุบัน + เดือนไทย พ.ศ. + คอลัมน์แหล่งข้อมูล · **งานถัดไปดู `PLAN.md`** (เหลือหลักๆ = D go-live รอ owner, redesign หน้าตายังไม่แตะ)
...
## Log งานเสริมรอบ 2026-07-20 (เดือนไทย + data_source + UI โหมดค่าเดียว) — ✅ เสร็จ

หลัง H1-J เสร็จ owner ขอปรับเพิ่ม 3 อย่าง (commit `9c90abd` + `b79049e`):

1. **เดือนไทย พ.ศ.** — `components/ThaiMonthInput.tsx` ใหม่ (dropdown เดือนไทย มกราคม-ธันวาคม + ปี พ.ศ.) แทน `<input type="month">` 3 จุด (`/kpi/[id]`, `/admin` แท็บ MOPH, `KpiWizard`) · ยังส่ง `YYYY-MM` (ค.ศ.) เหมือนเดิม คง min/max ล็อกเดือน staff · ช่อง "กำหนดเสร็จ" (`type=date`) **owner สั่งปล่อยไว้ก่อน** ยังเป็นปฏิทินเบราว์เซอร์

2. **คอลัมน์แหล่งข้อมูล `data_source`** — schema เพิ่ม `kpi_reports.data_source VARCHAR(50) NOT NULL DEFAULT 'HDC'` (provenance = ข้อมูล**มาจากไหน** · ต่างจาก `manual_entry` = กลไก**ได้มายังไง**) · wire types + `/api/kpis` GET/POST + `[id]` GET/PUT + ฟอร์ม `/admin` (ช่องแก้ + badge "📊 <source>" ในตาราง) · ทดสอบ init บน DB เปล่าผ่าน
   - **⚠️ owner ตัดสิน 2026-07-20: คงทุกตัวเป็น `HDC` หมด** (ทั้ง 38 auto + 2 manual) — ไม่แยกตัวกรอกเองเป็นป้ายอื่น เพราะ "ที่มาของตัวเลข" คือ HDC เหมือนกัน (เจ้าหน้าที่ก๊อปเลขจากจอ HDC มากรอก) ต่างแค่วิธีเอาเข้าระบบ · คอลัมน์นี้ไว้เผื่ออนาคตมีแหล่งที่ 3 จริง (API อื่น/ระบบ รพ.) ค่อยเปลี่ยน · ปิดช่องว่างที่ `kpi-work-groups-plan.md` เคยเขียนเผื่อไว้ (YAGNI เดิม)

3. **UI โหมดค่าเดียว (`manualScope='single'`)** — owner ขอให้เหมือนโหมดรายหน่วยบริการ (commit `b79049e`):
   - เปลี่ยนจากฟอร์ม 2 ช่องแนวนอน → **กราฟแท่ง + ตาราง** มีแถวเดียว `SINGLE_MODE_UNIT_NAME = 'โรงพยาบาลดงเจริญ'` + กราฟแท่งกึ่งกลาง + เส้นเป้าหมาย (reuse pattern เดียวกับตารางรายหน่วยบริการ)
   - **เปลี่ยนป้าย "ฐาน (B)" → "กลุ่มเป้าหมาย" ทั้ง 2 โหมด** (owner ยืนยันให้เหมือนกันทั้งระบบ)
   - verify ผ่าน browser จริงกับ KPI "test1" ของ owner: แก้ตัวเลขในตาราง → % + กราฟ + สถานะ อัปเดตสด → ยกเลิกคืนค่าเดิมถูกต้อง · KPI วัคซีน (unit mode) หัวตารางเปลี่ยนตามด้วย ข้อมูลเดิมไม่กระทบ

**หมายเหตุ go-live:** `/api/init` ตอนนี้เพิ่ม 2 คอลัมน์ใหม่ (`manual_scope`, `data_source`) รวมของค้างเดิม (4 ตาราง + remap `users.department`) — ทดสอบ init บน DB เปล่าผ่านหมดแล้ว รอรันจริงบน production ตอน go-live

---

## 🔴🟢 Phase 1 (ของแผน "เก็บ A/B ดิบ") — แก้บั๊ก single-mode ทำข้อมูลเพี้ยนเงียบตอนแก้ไขซ้ำ (6 ส.ค. 2569)

**เจอระหว่างทดสอบกรอกข้อมูลจริงกับ KPI #1 (อุบัติเหตุเด็ก/เยาวชน)** — ไม่ใช่บั๊กใหม่ เป็นมาตั้งแต่ commit `9c90abd`/`cd0738c` (Phase I เดิม) แค่ไม่เคยถูกจับได้

### สาเหตุ
`monthly_data` (มีแค่ `value`=%กับ `target`) เดิมเก็บ**แค่ %ที่คำนวณได้** — ไม่เก็บ "กลุ่มเป้าหมาย (B)"/"ผลงาน (A)" ดิบที่ผู้ใช้กรอกจริงเลย และ `target` column ในตารางนี้ = **เป้าหมายของตัว KPI เอง** (ใช้ตัดสินผ่าน/ไม่ผ่าน) **ไม่ใช่** B ที่กรอก — เป็นคนละตัวกันแท้ๆ

พอเปิดฟอร์มแก้ไขซ้ำ (`app/kpi/[id]/page.tsx` เดิม บรรทัด 187-188) ระบบ **เดา** B คืนมาจาก `kpi.target` (ผิดตัวเลข) แล้วเดา A ย้อนกลับด้วย `Math.round(value/100 × kpi.target)` (ปัดเศษซ้อน) → เปิดฟอร์มใหม่กี่ครั้งค่ายิ่งเพี้ยนสะสม (ทดสอบจริง: บันทึก 23.08% ปิดหน้าเปิดใหม่กลายเป็น 33.33%) **กระทบทุก KPI แบบ `manualScope='single'` ที่เป็นตัวเลข** (ตอนนั้นมี 21 ตัว) — ตัวที่ B ใกล้เคียงเป้า KPI โดยบังเอิญจะเพี้ยนน้อยจนสังเกตไม่ออก (เช่น #22 บันทึก 83% เปิดใหม่จะกลายเป็น 82.67%) อันตรายกว่าตัวที่เพี้ยนชัด เพราะดูเผินๆ เหมือนไม่มีอะไรผิด

### แก้
1. `monthly_data` เพิ่ม 2 คอลัมน์ `raw_target`/`raw_result` DECIMAL(14,2) NULL — เก็บ B/A ดิบที่กรอกจริงคู่กับ % เสมอ (`app/api/init/route.ts`)
2. `POST /api/monthly/single` เขียนค่าลง 2 คอลัมน์ใหม่ทุกครั้งที่บันทึก (numeric เท่านั้น — text/level เป็น NULL) + เก็บลง `data_change_log.old_data` ตอน overwrite ด้วย (audit ครบขึ้น)
3. `GET /api/detail` (branch `manualScope==='single'`) ส่ง `rawTarget`/`rawResult` กลับมาด้วย
4. `app/kpi/[id]/page.tsx`:
   - เลิกเดาค่าจาก % — ถ้ามี raw ใช้ raw ตรงๆ, **ถ้าไม่มี (แถวเก่าก่อนมีคอลัมน์นี้) เว้นฟอร์มว่างไว้ ไม่เดา** (`legacyNoRaw` flag) + ขึ้นแบนเนอร์เตือนว่าต้องกรอกใหม่ทั้งคู่
   - แยกแหล่งความจริงของ **การ์ดสรุป/กราฟ/badge สถานะ** ออกจากฟอร์มกรอก — ใช้ `committedPct`/`committedStatus` (มาจาก `data.savedMonthly.value` ตรงๆ) เสมอ ไม่ใช่ `sPct` ที่ผูกกับฟอร์ม (กันแถวเก่าที่ยังไม่มี raw โชว์ "—" ทั้งที่ % ถูกอยู่แล้วใน DB)
   - กันบันทึกทับ 0/0 เงียบๆ — ถ้าเป็นแถว legacy (มี % เดิมแต่ไม่มี raw) แล้วกด "บันทึกผลงาน" ทั้งที่ยังไม่ได้กรอกอะไรใหม่เลย ระบบบล็อก + บอกว่าค่าเดิมกี่ % จะไม่ถูกใช้ต่ออัตโนมัติ
   - เพิ่มการ์ด "นิยาม/วิธีกรอก" ให้โหมด single ด้วย (เดิมมีแต่โหมด auto/HDC — ทำให้คำอธิบายที่ตั้งไว้ใน `kpi_reports.description` ไม่เคยโชว์ให้ staff เห็นเลยสำหรับ KPI กรอกมือค่าเดียวทุกตัว)

### verify
- `npx tsc --noEmit` ผ่านสะอาด (ไม่ได้รัน `next build` เพราะ dev server เปิดอยู่ — กฎ HANDOFF)
- browser จริง KPI #1: กรอก B=15.6/A=3.6 → บันทึก 23.08% → **ปิดหน้าเปิดใหม่ยังคง 23.08% + B/A ครบ** (เดิมจะกลายเป็น 33.33%) · กด "แก้ไข" ซ้ำ prefill ถูกต้อง 15.6/3.6
- KPI #22 (legacy, บันทึกไว้ 83% ก่อนมีคอลัมน์นี้): การ์ดสรุปยังโชว์ 83%/ผ่านถูกต้อง (ไม่หายเป็น "—") + ตารางโชว่าง "—" ไม่เดาเลข + แบนเนอร์เตือนขึ้นถูก + กด "แก้ไข"→"บันทึกผลงาน" ทันทีโดยไม่กรอกอะไร → **ระบบบล็อกไม่ให้ทับ 83% ด้วย 0%** (regression check ผ่าน — ข้อมูลเดิมไม่ขยับ)
- regression: `/dashboard`, `/sets/inspection-r3`, KPI โหมด unit (`kpi-1780811100432` วัคซีน) — 0 console error, เลขตรงเหมือนเดิมทุกตัว
- backup ก่อนแก้: `_resync_backup/gongB-target-fix-2026-08-06/` (คนละงานกับที่แก้เป้า 5 ตัวก่อนหน้า แต่ทำต่อเนื่องกันวันเดียว)

---

## 🔴🟢 แก้เส้นทาง "API ล่ม → กรอกมือชั่วคราว → API กลับมา" (6 ส.ค. 2569, ต่อจาก Phase 1)

owner ถามว่า "ถ้า MOPH API ล่มแล้วอยากกรอกมือ ระบบเดิมโอเคไหม" → ตรวจ code path จริง + **ทดสอบสลับ auto↔manual กับ KPI จริง 2 ตัว** (คืนค่าครบ เทียบ backup ต่างแค่ `updated_at`)

### ✅ ที่ดีอยู่แล้ว (ยืนยันแล้ว ไม่ต้องแก้)
- ติ๊ก "กรอกค่าเอง" → cron/batch ข้ามทันที (`if (isManualEntry) skip` ใน `lib/mophBatch.ts`)
- **API ล่มสนิท (404) ไม่ทำข้อมูลเดิมหาย** — โยน error ก่อนเขียน DB ทุกเส้น
- **API คืน array ว่าง (200) ก็ไม่ทำ snapshot หาย** — `saveMonthlyDetail` มี guard `rows.length===0` / `groups.size===0` return ก่อนแตะ DB (จุดนี้เกือบเข้าใจผิดว่าเป็นบั๊ก — เช็คแล้วเขากันไว้ดีแล้ว)
- กรอกมือทับ auto → `data_change_log` เก็บทั้ง monthly + detail เดิม กู้คืนได้

### 🔴 บั๊ก 1 — auto ทับค่าที่คนกรอกมือ **แบบเงียบ ไม่มี audit เลย**
`/api/moph/route.ts` และ `lib/mophBatch.ts` **ไม่เขียน `data_change_log` สักบรรทัด** (ต่างจากเส้น manual ที่เขียนครบทุกจุด) → เคสจริง: MOPH ล่ม → ติ๊กกรอกมือ → คีย์ค่า → API กลับมา ปลดติ๊ก/กดดึงซ้ำ → **ค่าที่คนพิมพ์หายถาวร ไม่มีร่องรอย**

**แก้:** เพิ่ม `logAutoOverwriteIfManual(kpiId, month)` ใน `lib/mophDetail.ts` (ที่เดียว ใช้ร่วม 2 เส้น)
- log **เฉพาะ** เมื่อค่าเดิม `source='manual'` (auto ทับ auto = ข้อมูลชุดเดียวกัน ไม่ต้องเก็บ ไม่ให้ log บวม)
- เก็บทั้ง `monthly_data` (รวม `raw_target`/`raw_result` ของ Phase 1) และ `moph_monthly_detail` → รูปแบบเดียวกับเส้น manual กู้คืนวิธีเดียวกัน
- **ต้องเรียกก่อน `saveMonthlyDetail`** เพราะฟังก์ชันนั้น `DELETE` detail เดิมทิ้งก่อน insert (ลำดับสำคัญ)
- additive: audit พังต้องไม่บล็อกการดึงข้อมูลหลัก (`catch { return false }` แบบเดียวกับ `saveMonthlyDetail`)
- เพิ่ม warning ให้ admin เห็นใน response/`cron_log` ว่าเพิ่งทับของที่กรอกมือ + บอกว่ากู้คืนได้

### 🔴 บั๊ก 2 — ตาราง drilldown **ไม่มีคอลัมน์เลย** หลังกรอกมือ (10 ตาราง)
`/api/detail` กรองคอลัมน์ด้วย whitelist ที่ผูกกับ `moph_table` (`lib/detailLabels.ts`) แต่ข้อมูลที่คนกรอกเก็บเป็น `{target,result}` เสมอ → ตารางที่ field ชื่อคนละชุดจะกรองแล้วเหลือ **ศูนย์คอลัมน์**
กระทบ 10 ตาราง: `s_dental_0_5_cavity_free` (c/b/a) · `s_ncd_bp` (b1/r1/a1) · `s_dm_screen_n`+`s_ht_screen_n` (target1/result1) · `s_child0_5_pshyche_develop_coverage` (c_1..c_9) · `s_childdev_specialpp` · `s_tida4i`+`s_tida4i_o` · `s_kpi_ageing` · `s_telemed_hosp`
> ทดสอบ `s_dm_control` ตอนแรก**ไม่พัง** เพราะบังเอิญ field ชื่อ `target`/`result` ตรงกันพอดี — ต้องทดสอบกับตารางที่ชื่อไม่ตรงถึงจะเห็น

**แก้:** `manual` → ไม่ใช้ whitelist ของ `moph_table` เลย
- มี `target`/`result` (กรอกมือแล้ว) → โชว์ 2 คอลัมน์ + ป้ายไทย "กลุ่มเป้าหมาย (B)" / "ผลงาน (A)"
- ยังไม่เคยกรอกทับ (ยังเป็น snapshot auto เดิมที่ field คนละชุด) → fallback เป็น field ดิบทั้งหมด **ดีกว่าโชว์ตารางว่าง**
- กัน branch พิเศษ sepsis (`s_kpi_sepsis_septic`) ทำงานตอน manual ด้วย (`&& !manual`) — ไม่งั้นยุบ D/A/B/C จาก field ไตรมาสที่ไม่มีแล้ว ได้ 0 ทุกช่อง

### verify
- `npx tsc --noEmit` ผ่านสะอาด
- **บั๊ก 1:** จำลอง SQL ชุดเดียวกับในโค้ดบน DB จริง (เดือนทดสอบ `2026-09`) → จับ `source='manual'` ถูก · `old_data` เก็บ `value 55.50 / raw_target 200 / raw_result 111` ครบกู้คืนได้ · ล้างข้อมูลทดสอบเกลี้ยง (0/0) · **ยังไม่ได้ทดสอบ end-to-end เพราะ MOPH API ยัง 404** — เมื่อ API กลับมาควรทดสอบซ้ำจริง
- **บั๊ก 2:** ทดสอบกับ cavity-free จริง — สลับเป็น manual → กรอกมือลงเดือนว่าง `2026-09` (ไม่แตะ snapshot ส.ค. 53 แถว) → `fieldList: ['target','result']` + ป้ายไทยถูก + calcValue 76.67 ตรง · เดือน ส.ค. (snapshot เดิม c/b/a) fallback เป็น `['a','b','c']` ไม่ว่างเปล่า · ลบข้อมูลทดสอบ + คืนเป็น auto → snapshot 53 แถวครบ, มี.ค. manual + ส.ค. auto อยู่ครบ
- **regression:** sepsis ยังยุบ `D/A/B/C/A+C` = 15.38 · s_dm_control `target/hba1c/result` = 38.66 · cavity-free `c/b/a` = 70.54 · #23 `c_1..c_9` = 23.84 · วัคซีน (manual unit) `target/result` = 86.67 + ป้ายไทยดีขึ้น · browser 4 หน้า console 0 error
- backup: `_resync_backup/toggle-test-2026-08-06/`

### 🟡 ที่ยังไม่แก้ (flag ไว้)
1. **`manual_scope` ของ auto ทั้ง 41 ตัวเป็น `'unit'` หมด** (ค่า default ที่ไม่เคยตั้งจริง) → ติ๊ก manual ปุ๊บได้ฟอร์ม 7 หน่วยบริการทันที แม้ KPI นั้นเป็นระดับอำเภอ (เช่น #23 ที่พิสูจน์แล้วว่าไม่มีข้อมูลรายหน่วยจริง) — คนกรอกต้องรู้เองว่าต้องเปลี่ยนเป็น "ค่าเดียว" ก่อน · **แก้ตอน Phase 3 ได้พอดี** (จะแตะโหมดกรอกอยู่แล้ว) เช่น default เป็น `single` ถ้าอยู่ใน `DISTRICT_ONLY_TABLES`
2. **API "ป่วย" อันตรายกว่า API "ตาย"** — ถ้า MOPH คืนแค่ 2 จาก 5 ตำบล ระบบรับไปเต็มๆ (snapshot ถูกแทนด้วยข้อมูลบางส่วน + % ตกเงียบ) แยกไม่ออกจาก "ผลงานลดลงจริง" · ยังไม่มี guard แบบ "แถวหายเกิน X% เทียบเดือนก่อน → เตือนก่อนเขียน" — backlog

---

## 🔴🟢 Phase 3 (บางส่วน) — เพิ่ม `rate_per` รองรับหน่วยนอกจากร้อยละ + ตั้งค่าข้อ 21 (11 ส.ค. 2569)

owner ขอทำข้อ 21 (อัตราการฆ่าตัวตายสำเร็จ) ก่อน — เจอว่าไฟล์ตรวจราชการเดิมเขียนชื่อ/ตัวชี้วัดผิดตัว (ปนตัวย่อย "ร้อยละเข้าถึงบริการ") owner หา URL เอกสารทางการ (`inspection.moph.go.th/uploads/kpi_files/kpi_22.pdf`) มายืนยัน → ต้องใช้**ตัวหลัก** (crude suicide rate, หน่วยต่อแสนประชากร) ซึ่งชนกับข้อจำกัดเดิม (engine คูณ 100 แข็งทุก KPI) → ทำ Phase 3 เฉพาะส่วนที่ข้อ 21 ต้องใช้ก่อน (ไม่ทำเต็ม Phase 3 ทั้งหมด — ยังไม่แตะโหมด "จำนวนนับ")

### แก้
1. `kpi_reports` เพิ่มคอลัมน์ `rate_per` DECIMAL(12,2) NOT NULL DEFAULT 100 — ตัวคูณ A/B ก่อนแสดงผล (`app/api/init/route.ts`)
2. `POST /api/monthly/single` อ่าน `rate_per` มาคูณแทน `100` ที่ hardcode ไว้เดิม
3. `GET /api/detail` (branch single) ส่ง `ratePer` กลับมาด้วย
4. `app/kpi/[id]/page.tsx` — live-preview คำนวณด้วย `ratePer` จริง (ไม่ hardcode 100) + ข้อความ/หัวตารางเปลี่ยนตามหน่วยอัตโนมัติเมื่อ `ratePer≠100` (เช่น "ต่อแสนประชากร (A/B×100,000)" แทน "% (A/B)")
5. `app/api/kpis/route.ts` (list) + `[id]/route.ts` (GET/PUT) + `lib/types.ts` — wire `ratePer` ให้ครบเหมือน field อื่น (PUT ใช้ pattern `COALESCE` เดียวกับ `reportFreq` — ไม่ส่งมา = คงค่าเดิม กันฟอร์มเก่ารีเซ็ตเงียบ)
6. `app/admin/page.tsx` — เพิ่ม dropdown "วิธีคิดค่าที่กรอก" (ร้อยละ/ต่อพัน/ต่อหมื่น/ต่อแสนประชากร/ต่อล้าน) โชว์เฉพาะ KPI ที่เป็น manual+single+numeric

### 🔴 บั๊กที่เจอ+แก้ระหว่าง verify — DECIMAL column กลับมาเป็น string ทำ `<select>` ไม่ pre-select
mysql2 คืนค่าคอลัมน์ `DECIMAL` เป็น **string** เสมอ (เช่น `"100000.00"` ไม่ใช่ number `100000`) — `target` ก็เป็นแบบนี้เหมือนกันแต่ไม่มีใครเจอเพราะ `<input type=number>` ทนกับ string ได้ ส่วน `<select value=...>` ต้อง match string ตรงเป๊ะกับ `<option value>` → `"100000.00" !== "100000"` ทำให้ dropdown เด้งกลับตัวเลือกแรก (ร้อยละ) เงียบๆ ทั้งที่ DB ถูกอยู่แล้ว
**แก้:** `openEdit()` ใน `app/admin/page.tsx` ต้อง `Number(kpi.ratePer) || 100` เสมอ ก่อนใส่ลง form state — **บทเรียน: ทุกครั้งที่เอาค่าจาก DECIMAL column ไปใช้กับ `<select>` ต้อง `Number()` ก่อน** (ค่าอื่นที่เป็น DECIMAL แล้วยังไม่เคยเข้า select เช่น `target` ยังไม่มีปัญหานี้ แต่ถ้าวันไหนเอาไปทำ select ก็ต้องระวังแบบเดียวกัน)

### ตั้งค่าข้อ 21
`kpi-1785354100574`: ชื่อตัด "1) ร้อยละเข้าถึงบริการ" ออก (เหลือแค่ตัวหลัก) · target=**7.8** · direction=**lte** (ยิ่งน้อยยิ่งดี) · unit=**ต่อแสนประชากร** · rate_per=**100000** · description อธิบายสูตร+วิธีกรอก B(ประชากร)/A(เสียชีวิต) ยกมาจากเอกสารทางการเป๊ะ · `kpi_set_items` (target_region/province/hospital) = 7.8 ทั้ง 3 ระดับ (เอกสารให้เป้าเดียวกันทั้งประเทศ ไม่มี breakdown ราย รพ.) · ลบแถว `2026-03` เดิม (ค่าของตัวย่อยที่ไม่ใช่ตัวชี้วัดนี้แล้ว — confirm กับ owner ก่อนลบแล้ว)

### verify
- `npx tsc --noEmit` ผ่านสะอาดทั้ง 2 รอบ (หลังต่อ engine + หลังแก้บั๊ก select)
- browser จริง: กรอก B=13,000/A=1 → live preview 7.69 ถูกต้อง → บันทึก → DB เก็บ `value=7.69, target=7.80(เกณฑ์ผ่าน/ไม่ผ่านของ KPI), raw_target=13000, raw_result=1` → ปิดเปิดหน้าใหม่ยัง 7.69/13,000/1/ผ่าน ครบ (Phase 1 ทำงานร่วมกับ Phase 3 ถูกต้อง) → ลบข้อมูลทดสอบแล้ว
- regression: KPI #1 (rate_per default=100) ยังเป็น % ปกติ · #12 (มารดาตาย ต่อแสนคน — ยังไม่ได้แก้ต่อ) ยัง rate_per=100 ตามเดิม (บั๊กเดิมยังอยู่ รอคิว) · dashboard/sets/inspection-r3 0 console error · 80 KPI ครบ
- ฟอร์ม admin: เปิดแก้ไขข้อ 21 → dropdown "วิธีคิดค่าที่กรอก" ขึ้น "ต่อแสนประชากร (A/B×100,000)" ถูกต้อง (verify ผ่าน `document.querySelector` เพราะปุ่ม "แก้ไข" ในตารางยาวคลิกด้วย coordinate ไม่นิ่ง — ใช้ JS หา element แม่นแทน)
- backup: `_resync_backup/item21-suicide-rate-2026-08-11/`

### ยังไม่ทำ (คงเหลือจากแผน Phase 3 เดิม)
- โหมด "จำนวนนับ" (กรอกเลขเดียวไม่ต้องหาตัวหาร) — ยังไม่ทำ
- ตั้งค่า #12 (มารดาตาย ต่อแสนคน) ให้ใช้ `rate_per=100000` — engine พร้อมแล้ว รอ owner สั่งทำต่อ (ตอนนี้ยังคำนวณผิดสเกลเหมือนเดิม แต่ยังไม่กระทบเพราะตาย 0 ราย)
- Phase 2 (19 ตัวข้อความ) ยังไม่เริ่ม — รอ owner ตอบ `docs/kpi-text19-review.md`

### ยังไม่ทำ (Phase 2 ของแผนเดิม)
- **Phase 2:** ไล่ 19 ตัวที่ยังเป็น `measure_type='text'` เทียบไฟล์ต้นฉบับทีละตัว (มีเลขจริง→แปลงเป็น numeric ได้เลย / เป็น "ระดับ"→ต้องถาม owner ลำดับขั้นก่อน / ไม่มีเลขจริง→คงเป็น text) — เอกสารพร้อมแล้วที่ `docs/kpi-text19-review.md` รอ owner ตอบ
