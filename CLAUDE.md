...
- **manual KPI (กรอกค่าเอง):** `lib/manualKpi.ts` (`isManualEntry` อ่าน flag `kpi_reports.manual_entry`) · ติ๊ก "กรอกค่าเอง" ในฟอร์ม `/admin` · `runBatchSave`/cron **ข้าม** ตัว manual · กรอกรายตำบล (B/A) ที่หน้า `/kpi/[id]` (admin) → `POST /api/monthly/detail` เขียน `moph_monthly_detail` {target,result} + `monthly_data` รวม (ΣA/ΣB) + audit (`source/entered_by/entered_at`) · detail **บังคับ mapping result/target** เมื่อ manual
...
- **KPI กรอกค่าเอง (manual):** `s_epi_complete` (fully immunized) — Open Data คำนวณ per-child ไม่ได้ (ดู [`docs/kpi-verify-2569.md`](docs/kpi-verify-2569.md)) → owner กรอกรายตำบลจาก HDC เดือนละครั้ง · มี flag `manual_entry` ให้ติ๊ก KPI อื่นเพิ่มได้เอง
